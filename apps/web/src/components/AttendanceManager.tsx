...
import { Search, Filter, Calendar, Trash2, Eye, X, Users, TrendingUp, Brain } from 'lucide-react';
...
export default function AttendanceManager() {
  ...
  // Filter logs based on search query, model, and date range
  const filteredLogs = useMemo(() => {
    ...
    return filtered;
  }, [logs, searchQuery, modelFilter, startDate, endDate]);
  ...
  // Get badge color for accuracy
  const getAccuracyBadgeColor = (accuracy: number) => {
    if (accuracy >= 90) return 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400';
    if (accuracy >= 75) return 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400';
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center space-x-2">
              ...
              <div>
                <p className="text-sm text-muted-foreground">Model Utama</p>
                <p className="text-sm font-semibold capitalize">{stats.mostUsedModel}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <CardHeader>
          ...
          {showFilters && (
            <div className="border-t pt-4 mt-4">
              ...
              {(searchQuery || modelFilter !== 'all' || startDate || endDate) && (
                <div className="mt-4">
                  <Button 
                    variant="outline" 
                    size="sm"
                      ...
                      {filteredLogs.map((log) => (
                        <tr key={log.id} className="border-b hover:bg-muted/50 transition-colors">
                          ...
                          <td className="p-3 align-middle">
                            <Badge 
                              variant="secondary" 
                              className={getAccuracyBadgeColor((log.similarity * 100))}
                            >
                              {(log.similarity * 100).toFixed(1)}%
                            </Badge>
                          </td>
                          <td className="p-3 align-middle text-muted-foreground hidden lg:table-cell">
                            {log.latencyMs.toFixed(0)}ms
                          </td>
                          <td className="p-3 align-middle hidden md:table-cell">
                            <Badge 
                              variant="secondary" 
                              className={getModelBadgeColor(log.model)}
                            >
                              {log.model}
                            </Badge>
                          </td>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          {selectedLog && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-sm font-medium">Akurasi</Label>
                  <p className="text-sm">
                    <Badge className={getAccuracyBadgeColor((selectedLog.similarity * 100))}>
                      {(selectedLog.similarity * 100).toFixed(2)}%
                    </Badge>
                  </p>
                </div>
                <div>
                  <Label className="text-sm font-medium">Latency</Label>
                  <p className="text-sm">{selectedLog.latencyMs.toFixed(2)} ms</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm">
                    <Badge className={getModelBadgeColor(selectedLog.model)}>
                      {selectedLog.model}
                    </Badge>
                  </p>
                </div>
