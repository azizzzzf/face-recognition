    ...
    try:
        ...
        try:
            ...
            if arcface_embedding is not None:
                ...
                logger.warning(f"ArcFace could not detect faces for {request.name}, continuing with Face-API only")
        except Exception as arcface_error:
            logger.warning(f"ArcFace processing failed for {request.name}: {str(arcface_error)}, continuing with Face-API only")
        ...
        try:
            ...
            if test_embedding is not None:
                if user['arcface_descriptor']:
                    ...
                    logger.warning(f"User {user['name']} has no ArcFace descriptor registered")
            else:
        ...
        for user in users:
            ...
            try:
                ...
                if not enrollment_images:
                    logger.warning(f"User {user['name']} has no enrollment images")
